...
subscriber = User.objects.get(pk=1)
subscribed_to = User.objects.get(pk=2)
...
for subscription in subscriptions:
    print(f"User {user.username} is subscribed to {subscription.subscribed_to.username}")

# Получаем всех подписчиков пользователя
subscribed_user = User.objects.get(pk=2)
subscribers = subscribed_user.subscribers.all()

for subscriber in subscribers:
    print(f"User {subscriber.username} is subscribed to {subscribed_user.username}")
"""
...
# Создание поста
post = Post.objects.create(title='Introduction to Django', content='Content about Django.')

# Связывание поста с тегами
post.tags.add(tag1, tag2)  # Добавление нескольких тегов сразу

# Получение всех тегов для поста
tags_for_post = post.tags.all()

# Получение всех постов с определённым тегом
posts_with_tag = Tag.objects.get(name='Python').posts.all()
